...
To use the `LList` library, just place the command `$USELIBRARY:'a740g/QBDS-LList'` at the top of your project.
...
node = LList_GetFrontNode(myList())
DO WHILE node
...
Returns the data type of the value associated with a node. See [QBDS Common](../QBDS/Common.md) constants.
...
To store and retrieve **UDTs** in the list, you need to write wrapper **SUBs** around the low-level and public routines. These wrappers handle packing and unpacking the UDT into a raw string buffer.
